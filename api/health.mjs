...
async function handler(req, res) {
  ...
  const dbStatus = neonUrl || dbUrl ? `\u2705 Configurada (${neonUrl ? "NEON_DATABASE_URL" : "DATABASE_URL"})` : "\u274C N\xC3O configurada!";
  let dbConnection = "\u23F3 Testando...";
  try {
    if (neonUrl || dbUrl) {
      const postgres2 = (await Promise.resolve().then(() => (init_src(), src_exports))).default;
      const sql = postgres2(neonUrl || dbUrl, { max: 1, connect_timeout: 5 });
      await sql`SELECT 1 as ok`;
      await sql.end();
      dbConnection = "\u2705 Conex\xE3o com banco OK!";
    } else {
      dbConnection = "\u274C Sem URL de banco configurada";
    }
  } catch (err) {
    dbConnection = `\u274C Erro de conex\xE3o: ${err?.message}`;
  }
  res.status(200).json({
    status: "online",
