...
async function systemAuthMiddleware(c: any, next: any) {
  ...
  const session = await c.env.DB.prepare(
    "SELECT s.*, u.*, a.name as agency_name FROM user_sessions s JOIN system_users u ON s.user_id = u.id JOIN agencies a ON u.agency_id = a.id WHERE s.session_token = ? AND s.expires_at > datetime('now') AND u.is_active = 1"
  ...
  try {
    ...
    for (const step of defaultSteps) {
      await c.env.DB.prepare(
        "INSERT INTO process_steps (agency_id, name, type, sort_order, is_active) VALUES (?, ?, ?, ?, 1)"
      ).bind(agencyResult.id, step.name, step.type, step.sort_order).run();
    }
    ...
    for (const fee of defaultFees) {
      await c.env.DB.prepare(
        "INSERT INTO fees (agency_id, name, amount, is_active) VALUES (?, ?, ?, 1)"
      ).bind(agencyResult.id, fee.name, fee.amount).run();
    }
...
app.get("/api/cities", systemAuthMiddleware, async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM cities WHERE agency_id = ? AND is_active = 1 ORDER BY name"
  ).bind(user.agency_id).all();
...
app.delete("/api/cities/:id", systemAuthMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE cities SET is_active = 0 WHERE id = ? AND agency_id = ?"
...
app.get("/api/professionals", systemAuthMiddleware, async (c) => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    WHERE p.agency_id = ? AND p.is_active = 1 
    ORDER BY p.name
  `).bind(user.agency_id).all();
...
app.delete("/api/professionals/:id", systemAuthMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE professionals SET is_active = 0 WHERE id = ? AND agency_id = ?"
...
app.patch("/api/process-steps/:id", systemAuthMiddleware, async (c) => {
  ...
  if (body.is_active !== undefined) {
    ...
    ).bind(body.is_active ? 1 : 0, stepId, user.agency_id).run();
  }
...
app.post("/api/step-processes", systemAuthMiddleware, async (c) => {
  try {
    ...
    const { results: allFees } = await c.env.DB.prepare(
      "SELECT * FROM fees WHERE agency_id = ? AND is_active = 1 AND linked_professional_type IS NOT NULL"
    ).bind(user.agency_id).all();
...
app.get("/api/step-processes/:id", systemAuthMiddleware, async (c) => {
  ...
  const { results: allFees } = await c.env.DB.prepare(
    "SELECT * FROM fees WHERE agency_id = ? AND is_active = 1"
  ).bind(user.agency_id).all();
